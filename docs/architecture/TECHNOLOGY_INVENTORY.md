...
| Groq SDK (`groq-sdk`) | backend (`server/ai/providers/groqProvider.js`) | **The live default provider** for all 6 pipeline operations (Phase 1B) | `openai/gpt-oss-120b` is one of only two Groq models with strict-mode (constrained-decoding) JSON Schema output | Anthropic's direct `fetch()`-based integration has been fully removed — no runtime code, dependency, or environment variable for it remains |
| Google Gen AI SDK (`@google/genai`) | backend (`server/ai/providers/geminiProvider.js`) | **Live, optional alternative provider**, selected via `AI_PROVIDER=gemini` | For controlled comparison and synthetic-data experimentation only — see the Gemini data-use restriction in ADR-0002 | `GEMINI_MODEL` has no built-in default (model IDs change too often to hardcode) — the process fails to construct a Gemini provider without one |
| Vitest | testing | Test runner | Native fit for Vite projects | Frontend: 11 tests (was 1 placeholder). Backend: 159 tests across scoring, schemas, providers, pipeline, and HTTP/SSE routes (was 0 — no config covered `server/` before Phase 1A) |
...
| Zod | dependency | Schema validation | Common TypeScript validation library | **Live**: all 6 production LLM operations are validated against a Zod schema (`server/ai/schemas/`) before any deterministic calculation runs. No longer "installed but unused" in any sense |
...
3. **Model integration** — a provider interface was introduced in Phase 1A (`server/ai/`) and the live pipeline was wired onto it in Phase 1B; the Anthropic-specific integration it replaced has been fully removed.
