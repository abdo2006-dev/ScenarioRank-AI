...
| Vitest | testing | Test runner | Native fit for Vite projects | Frontend: 16 tests (was 1 placeholder). Backend: 155 tests across scoring, schemas, the OpenAI adapter, pipeline (including batching/request-budget behavior), and HTTP/SSE routes (was 0 — no config covered `server/` before Phase 1A) |
