 ...
 * never be softened.
 *
 * Runs after pairing (docs/decisions/ADR-0004-single-openai-provider.md,
 * request-count reduction reorders pairing before the decision-explanation
 * call), so `pairingSummary` — a plain-language, already-deterministic
 * description of the pairing result, never model-invented — can be woven
 * into the narrative when pairing was enabled and available. This is
 * prompt context only, not a new schema field: the pairing result itself
 * is already fully known before this call, so the model is not asked to
 * compute or validate anything about it, only to acknowledge it in prose.
 * @param {{ roleTitle: string, scenario: string, modeLabel: string, winnerName: string, rankedSummary: string, pairingSummary?: string }} input
 */
...
${input.rankedSummary}
${input.pairingSummary ? `\nPairing result (already computed, do not change it): ${input.pairingSummary}` : ""}
